...
function toggleAddDialog() {
  groundhogApp.addDialogContainer.classList.toggle('visible');
}
...
function addLocation() {
  ...
  groundhogApp.selectedLocations[geo] = location;
  saveLocationList(groundhogApp.selectedLocations);
}

function removeLocation(evt) {
  const parent = evt.srcElement.parentElement;
  parent.remove();
  if (groundhogApp.selectedLocations[parent.id]) {
    delete groundhogApp.selectedLocations[parent.id];
    saveLocationList(groundhogApp.selectedLocations);
  }
}
...
function updateData() {
  Object.keys(groundhogApp.selectedLocations).forEach((key) => {
    const location = groundhogApp.selectedLocations[key];
    const card = getForecastCard(location);
...
function init() {
  groundhogApp.selectedLocations = loadLocationList();
  updateData();
